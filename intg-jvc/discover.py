...
class JVCProjectorDiscovery(SDDPDiscovery):
    """Discover JVC Projectors in local network using SDDP protocol."""
    ...
    def parse_sddp_response(
        ...
        try:
            ...
            device_type = (
                datagram.hdr_type if hasattr(datagram, "hdr_type") else "JVC Projector"
            )

            identifier = f"jvc_{ip_address.replace('.', '_')}"

            # Use device type as the name
            device_name = device_type if device_type else "JVC Projector"

            _LOG.debug(
                "Parsed JVC Projector: %s at %s (type: %s)",
                device_name,
                ip_address,
                device_type,
            )
